...
// Get lowest two octets of the first v4 IP address enumerated, or
// two random digits in the case of error.
//
// TODO This is not perfect! Bugs below.
//
// - FlagUp bug in Windows? Always returns 0. Sent Luis an email. Believe it's because
//   WSAIoctl SIO_GET_INTERFACE_LIST doesn't set iiFlags to IFF_UP on a synthNIC
//
...
func getOctets() (octet1 byte, octet2 byte) {
	...
	for _, i := range ifaces {
		//if i.Flags&net.FlagUp == net.FlagUp {  // In a VM, flags always==0. Bug???
					...
					if ip := net.ParseIP(addr.String()); ip != nil {
						if ip.String() != "0.0.0.0" {
							if v4addr := net.IP.To4(ip); v4addr != nil {
								if len(ip) == 16 {
									log.Debugf("Seed MAC using %d.%d from %s on %s", ip[14], ip[15], ip.String(), i.HardwareAddr)
									return ip[14], ip[15]
								}
							}
						}
	...
	log.Warningln("Failed to find an IPv4 address to set MAC range. Using random seed instead")
	return randomByte(), randomByte()
}

// This should be called by the driver in it's init processing
...
func SetupMACRange(ouiPrefix []byte) (err error) {
	...
	last = endMACRange
	log.Debugf("MAC address range %v:... addresses %d", macBase, endMACRange)
	return nil
}
